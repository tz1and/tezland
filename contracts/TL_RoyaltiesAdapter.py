...
class TL_RoyaltiesAdapter(sp.Contract):
    def __init__(self, registry, legacy_royalties, metadata, exception_optimization_level="default-line"):
    ...
    def get_token_royalties(self, params):
        ...
        royalties_type = sp.local("royalties_type", sp.view("get_royalties_type", self.data.registry,
            params.fa2, t = registry_contract.t_get_royalties_type_result).open_some())
        ...
        with sp.if_(royalties_type.value == 0):
            ...
            sp.result(sp.view("get_token_royalties", self.data.legacy_royalties,
                params, t = FA2.t_royalties_interop).open_some())

        with sp.else_():
            with sp.if_(royalties_type.value == 1):
                # Convert V1 royalties to V2.
                royalties = sp.compute(FA2_legacy.get_token_royalties(params.fa2, params.id))
            ...
            with sp.else_():
                with sp.if_(royalties_type.value == 2):
                    ...
                    sp.result(FA2.get_token_royalties(params.fa2, params.id))
                with sp.else_():
