    ...
    def __init__(self):
        ...
        if hasattr(self, 'available_settings'):
            self.available_settings.append(
                ("moderation_contract", sp.TOption(sp.TAddress), lambda x : utils.ifSomeRun(x, lambda y: utils.isContract(y)))
            )
        else:
            def set_moderation_contract(self, moderation_contract):
                ...
                utils.ifSomeRun(moderation_contract, lambda y: utils.isContract(y))
