...
@sp.add_test(name = "GenericLambdaProxy_tests_places", profile = True)
def test():
    ...
    scenario.h2("Test GenericLambdaProxy for Place tokens")
    ...
    base = Tokens.PlaceTokenProxyBase(admin.address,
        metadata=sp.utils.metadata_of_url("ipfs://example"),
        admin=admin.address, blacklist=blacklist.address, name="", description="")
    scenario += base

    scenario.h4("ProxyParent")
    parent = Tokens.PlaceTokenProxyParent(admin.address,
        metadata=sp.utils.metadata_of_url("ipfs://example"),
        admin=admin.address, blacklist=blacklist.address, name="", description="")
    scenario += parent

    scenario.h4("ProxyChild")
    child = Tokens.PlaceTokenProxyChild(parent.address,
        metadata=sp.utils.metadata_of_url("ipfs://example"),
        admin=admin.address, blacklist=blacklist.address, name="", description="")
    scenario += child
    ...
    upgrade_transfer = sp.record(id = sp.contract_entrypoint_id(parent, "transfer_tokens"), new_code = sp.utils.wrap_entry_point("transfer_tokens", test_transfer_update))
...
@sp.add_test(name = "GenericLambdaProxy_tests_collections", profile = True)
def test():
    admin = sp.test_account("Administrator")
    alice = sp.test_account("Alice")
    bob   = sp.test_account("Robert")
    ...
    scenario.h2("Test GenericLambdaProxy for Collections")
