...
def test():
    ...
    sp.verify(whitelist.whitelist_enabled() == False)
    whitelist.manage_whitelist(sp.variant("whitelist_enabled", True)).run(sender=admin)
    sp.verify(whitelist.whitelist_enabled() == True)

    scenario.h4("is_whitelisted")
    sp.verify(whitelist.is_whitelisted(bob.address) == False)
    sp.verify(whitelist.is_whitelisted(alice.address) == False)
    sp.verify(whitelist.is_whitelisted(admin.address) == False)

    whitelist.manage_whitelist(sp.variant("whitelist_add", [bob.address, admin.address])).run(sender=admin)
    sp.verify(whitelist.is_whitelisted(bob.address) == True)
    sp.verify(whitelist.is_whitelisted(alice.address) == False)
    sp.verify(whitelist.is_whitelisted(admin.address) == True)
