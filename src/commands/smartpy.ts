...
export function compile_newtarget(contract_name: string, target_args: string[]): void {
    console.log(kleur.yellow(`Compiling contract '${contract_name}' ...`));
    ...
    console.log(`Building ${contract_name} target`)
    ...
    fs.writeFileSync(`./${target_out_dir}/${contract_name}_target.py`, `import smartpy as sp
${contract_name}_contract = sp.io.import_script_from_url("file:contracts/${contract_name}.py")
sp.add_compilation_target("${contract_name}", ${contract_name}_contract.${contract_name}(
    ${target_args.join(', ')}
    ))`)
    ...
    const contract_in = `${target_out_dir}/${contract_name}_target.py`
